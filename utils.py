...
def at(x):
    """
    计算注意力映射。公式：将输入 x 沿 channel 维度求平方均值，
    然后 reshape 成 (batch, -1)，最后进行 L2 归一化。
    """
    # x: [N, C, H, W]
    att = x.pow(2).mean(1).view(x.size(0), -1)  # [N, H*W]
    return F.normalize(att, p=2, dim=1)

def at_loss(student_feature, teacher_feature):
    """
    计算注意力转移损失（AT Loss），即两个注意力映射之间的均方误差。
    """
    return (at(student_feature) - at(teacher_feature)).pow(2).mean() * 1000
...
def gradient_flow_repair(model, low_threshold=1e-3, high_threshold=1.0):
    """
    对模型中所有参数的梯度进行检测和修复：
    1. 如果某个参数的梯度 L2 范数低于 low_threshold，则将其放大到 low_threshold；
    2. 如果梯度 L2 范数高于 high_threshold，则进行梯度裁剪到 high_threshold。
    
    参数：
        model: 当前模型（例如你的 VanillaNet-6 模型）
        low_threshold: 梯度下限阈值
        high_threshold: 梯度上限阈值
    """
    for name, param in model.named_parameters():
        if param.grad is not None:
            ...
            if grad_norm < low_threshold:
                scaling_factor = low_threshold / (grad_norm + 1e-8)
                param.grad.data.mul_(scaling_factor)
                # 可选：打印调试信息
                # print(f"[Gradient Repair] {name}: gradient norm {grad_norm:.4f} too low, scaled by {scaling_factor:.2f}")
            # 如果梯度太大，则裁剪
            elif grad_norm > high_threshold:
                scaling_factor = high_threshold / grad_norm
                param.grad.data.mul_(scaling_factor)
                # print(f"[Gradient Repair] {name}: gradient norm {grad_norm:.4f} too high, clipped by {scaling_factor:.2f}")
